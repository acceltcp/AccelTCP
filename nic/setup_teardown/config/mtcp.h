...
/* -------- mTCP NIC offload related parameters (experimental) ---------------*/
#define ACK_PIGGYBACK FALSE
#define ON_EXPERIMENT TRUE

/* (1) connection setup offload */
#define CONNECTION_SETUP_OFFLOAD TRUE
#define HASH_TABLE_OFFLOAD       FALSE
#define VERBOSE                  FALSE
#define RESET_CASE               FALSE

#if CONNECTION_SETUP_OFFLOAD
#define IPPROTO_CUSTOM 0xAA
#endif

#if HASH_TABLE_OFFLOAD
#define ETH_P_CUSTOM_ACK 0xAAAA
#endif
...
#define IP_CALCULATE_CHECKSUM       FALSE
#else
#define TCP_OPT_TIMESTAMP_ENABLED   TRUE
#define VERIFY_RX_CHECKSUM          TRUE
#define TCP_CALCULATE_CHECKSUM      TRUE
#define IP_CALCULATE_CHECKSUM       TRUE
#endif

/* (2) connection teardown offload */
#define CONNECTION_TEARDOWN_OFFLOAD	 TRUE
#define FORCE_FIN_PIGGYBACK			 TRUE
// -> (to be fixed) for now, it must be TRUE if CONNECTION_TEARDOWN_OFFLOAD is TRUE
#define FORCE_ACK_PIGGYBACK		     TRUE

/* (3) connection splice offload */
// #define CONNECTION_SPLICE_OFFLOAD	TRUE
// -> always enabled because we don't have mtcp_splice() w/o NIC offload for now

/* ethernet type used for NIC offload (teardown, splice) */
#define ETH_P_IP_TCP_SPLICE_OFFLOAD      0x0807
#if CONNECTION_TEARDOWN_OFFLOAD
#define ETH_P_IP_TCP_FIN_OFFLOAD_PASSIVE 0x0808
#define ETH_P_IP_TCP_FIN_OFFLOAD_ACTIVE  0x0809
#define ETH_P_IP_TCP_FIN_OFFLOAD_CONTROL 0x080A
#endif
...
#define ENABLETSO 1
#if ENABLETSO
#define MAX_TSO_BUFSIZE 10000
#endif
*/

/* increasing initial congestion window size to 10 MSS (rfc6928) */
//#define INIT_CWND_10 1

/* (5) mTCP SYN cookie at host (experimental) */
#define SYN_COOKIE FALSE /* Conducts SYN_COOKIE in mTCP */

/* This feature is activated only when SYN_COOKIE is FALSE */
#if !SYN_COOKIE
#define SETUP_TEST_NO_STATE FALSE /* Conducts SYN-SYNACK-ACK-RST without any state management in mTCP */
#define SETUP_TEST_NO_STATE_ISN 1234
#endif

/* (6) respond to SYN cookie ACK with FIN + HTTP response (experimental) */
#define TEST_BYPASS_MTCP TRUE
